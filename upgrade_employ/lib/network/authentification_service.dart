...
class AuthentificationService extends GetxController {
  ...
  Future<Response> changepasswordPut(Map<String, dynamic> data) async {
      final response = await getConnect.put('${Constante.apiPath}account/change-password/', data);
      return response;
  }

     Future<Response> ajouterUserPost(Map<String, dynamic> data) async {
      final response = await getConnect.post('${Constante.apiPath}account/create-user/', data);
      return response;
  }

  Future<Response> listeUseGet(String token) async {
    final response =
        await getConnect.get('${Constante.apiPath}account/liste-users/', headers: {
      'Authorization': 'Token $token',
    });
    return response;
  }
